...
class relatorioDAO
{
    ...
    public function relatorio03()
    {
        global $pdo;
        try {
            $statement = $pdo->prepare('SELECT tb_payments.id_payment, tb_city.str_name_city, tb_functions.str_name_function, tb_subfunctions.str_name_subfunction, tb_program.str_name_program, tb_action.str_name_action, tb_beneficiaries.str_name_person, tb_source.str_goal, tb_files.str_name_file FROM tb_payments, tb_city, tb_functions, tb_beneficiaries, tb_action, tb_program, tb_subfunctions, tb_source, tb_files where tb_payments.tb_city_id_city = tb_city.id_city and tb_payments.tb_beneficiaries_id_beneficiaries = tb_beneficiaries.id_beneficiaries and tb_payments.tb_action_id_action = tb_action.id_action and tb_payments.tb_program_id_program = tb_program.id_program and tb_payments.tb_subfunctions_id_subfunction = tb_subfunctions.id_subfunction and tb_payments.tb_source_id_source = tb_source.id_source and tb_payments.tb_functions_id_function = tb_functions.id_function and tb_payments.tb_files_id_file = tb_files.id_file;');
            if ($statement->execute()) {
                $lista = $statement->fetchAll(PDO::FETCH_OBJ);
                return $lista;
            }else {
    ...
    public function relatorio04()
    {
        global $pdo;
        try {
            $statement = $pdo->prepare('SELECT sum(db_value) as soma, tb_city.str_name_city , count(distinct tb_payments.tb_beneficiaries_id_beneficiaries) as contador FROM tb_payments, tb_city where tb_payments.id_payment = tb_city.id_city group by tb_payments.tb_city_id_city order by sum(db_value) DESC;');
            if ($statement->execute()) {
                $lista = $statement->fetchAll(PDO::FETCH_OBJ);
                return $lista;
            }else {
    ...
    public function relatorio06()
    {
        global $pdo;
        try {
            $statement = $pdo->prepare('SELECT sum(db_value) valor, str_name_region FROM db_eca.tb_payments, tb_city, tb_state, tb_region where tb_payments.id_payment = tb_city.id_city and tb_city.tb_state_id_state = tb_state.id_state and tb_state.tb_region_id_region = tb_region.id_region group by tb_region.id_region order by tb_state.str_name;');
            if ($statement->execute()) {
                $lista = $statement->fetchAll(PDO::FETCH_OBJ);
                return $lista;
            }else {
